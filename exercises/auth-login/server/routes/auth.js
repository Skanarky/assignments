...
authRouter.post("/login", (req, res) => {
    User.findOne({username: req.body.username.toLowerCase()}, (err, user) => {
        ...
        if (!user) {
            return res.status(403).send({success: false, message: "invalid user name"})
        }
        ...
        user.checkPassword(req.body.password, (err, match) => {
            ...
            return res.status(200).send({token: token, user: user.withoutPassword(), success: true, message: "Here's your token!"})
        })
